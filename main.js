...
function loadSettings() {
    ...
    try {
        ...
        addLog(`An error occured while loading Lyrics Status config!\nError:<br><span style="color: rgba(150, 0, 0, var(--alpha));user-select: text;">${e}</span>`, "error");
    }
}
...
function updatePlaybackState() {
    ...
    return $.ajax({
        ...
        statusCode: {
            ...
            404: () => { addLog("Got unexpected error! },
            502: () => { addLog("Got unexpected error! }
        }
    });
}
...
(async function playbackStateUpdater() {
    ...
    updatePlaybackState().always(async () => {
        if(errorCount >= 10) {
            addLog("Sky Lyrics has been stopped due to errors.", "warning");
            stopLog = true;
            stopped = true;
...
(async function statusChanger() {
    setInterval(() => {
        if(startLog) {
            ...
            debugRequest10.text("Fetching...");
            debugRequest30.text("Fetching...");
            addLog("Sky Lyrics started...");
        }
        if(stopLog) {
            stopLog = false;
            debugLyrics.text("Sky Lyrics is stopped");
            debugRequest.text("Sky Lyrics is stopped");
            debugRequest2.text("Sky Lyrics is stopped");
            debugRequest10.text("Sky Lyrics is stopped");
            debugRequest30.text("Sky Lyrics is stopped");
            addLog("Sky Lyrics stopped...");
        }
        if(stopped) {
